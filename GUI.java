...
public class GUI extends JFrame {
	...
	public GUI(DefaultTableModel defaultTableModel) {
		super();
	...
	private class ListenForAction implements ActionListener {
		public void actionPerformed(ActionEvent e) {
			if(e.getSource() == addCustomer) {
				...
				Customer.sqlDateRegistered = Customer.getADate(dateRegistered);
				
				int customerID = 0;
				
				try {
					Customer.rows.moveToInsertRow();
					Customer.rows.updateString("first_name", firstName);
					Customer.rows.updateString("last_name", lastName);
					Customer.rows.updateString("phone_number", phoneNumber);
					Customer.rows.updateString("email_address", emailAddress);
					Customer.rows.updateString("city", city);
					Customer.rows.updateString("state", state);
					Customer.rows.updateDate("date_registered", (Date) Customer.sqlDateRegistered);
					
					Customer.rows.insertRow();
					Customer.rows.updateRow();
					
					Customer.rows.last();
					customerID = Customer.rows.getInt(1);
					Object[] customer = {customerID, firstName, lastName, phoneNumber, emailAddress, city, state, Customer.sqlDateRegistered};
					Customer.defaultTableModel.addRow(customer);
				...
				try {
					Customer.defaultTableModel.removeRow(table.getSelectedRow());
					Customer.rows.absolute(table.getSelectedRow());
					Customer.rows.deleteRow();
			...
			public void mouseReleased(MouseEvent mouseEvent) {
				if (SwingUtilities.isRightMouseButton(mouseEvent)) {
					...
					if(value != null) {
						...
						try {
							Customer.rows.absolute(table.getSelectedRow()+1);
							updateColumn = Customer.defaultTableModel.getColumnName(table.getSelectedColumn());
						
							switch(updateColumn) {
							case "Date_Registered":
								Customer.sqlDateRegistered = Customer.getADate(value);
								Customer.rows.updateDate(updateColumn, (Date) Customer.sqlDateRegistered);
								Customer.rows.updateRow();
								break;
							default:
								Customer.rows.updateString(updateColumn, value);
								Customer.rows.updateRow();
								break;
							} 
						} catch (SQLException e1) {
